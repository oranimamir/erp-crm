...
router.post('/', (req: Request, res: Response) => {
  ...
  const insertOrder = db.transaction(() => {
    ...
    if (items && Array.isArray(items)) {
      const insertItem = db.prepare('INSERT INTO order_items (order_id, description, quantity, unit_price, total) VALUES (?, ?, ?, ?, ?)');
      for (const item of items) {
        insertItem.run(orderId, item.description, item.quantity, item.unit_price, item.quantity * item.unit_price);
      }
    }
...
router.put('/:id', (req: Request, res: Response) => {
  ...
  const updateOrder = db.transaction(() => {
    ...
    if (items && Array.isArray(items)) {
      ...
      const insertItem = db.prepare('INSERT INTO order_items (order_id, description, quantity, unit_price) VALUES (?, ?, ?, ?)');
      for (const item of items) {
        insertItem.run(req.params.id, item.description, item.quantity, item.unit_price);
      }
    }
