...
router.get('/', (req, res) => {
  ...
  if (search) {
    where += ' AND (type LIKE ? OR code LIKE ? OR product LIKE ?)';
...
// POST / — create
router.post('/', (req, res) => {
  const { type, code, product_mass, product, units_per_pallet, pallet_label_code, weight_per_pallet, weight_packaging, weight_pallet, gross_weight, compatible = 'Food', notes = '' } = req.body;
  ...
  try {
    const result = db.prepare(`
      INSERT INTO packaging (type, code, product_mass, product, units_per_pallet, pallet_label_code, weight_per_pallet, weight_packaging, weight_pallet, gross_weight, compatible, notes)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
router.put('/:id', (req, res) => {
  ...
  const { type, code, product_mass, product, units_per_pallet, pallet_label_code, weight_per_pallet, weight_packaging, weight_pallet, gross_weight, compatible, notes } = req.body;
  if (!type?.trim() || !code?.trim()) {
    res.status(400).json({ error: 'Type and Code are required' });
    return;
  }
