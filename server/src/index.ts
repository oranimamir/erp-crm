...
import packingListRoutes, { backfillPackingListFiling } from './routes/packing-lists.js';
...
import blScanRoutes from './routes/bl-scan.js';
...
app.use('/api/order-confirmations', authenticateToken, orderConfirmationRoutes);
app.use('/api/invoice-documents', authenticateToken, invoiceDocumentRoutes);
app.use('/api/purchase-orders', authenticateToken, purchaseOrderRoutes);
app.use('/api/packing-lists', authenticateToken, packingListRoutes);
app.use('/api/company-entities', authenticateToken, companyEntityRoutes);
...
app.use('/api/bl-scan', authenticateToken, blScanRoutes);
...
const server = app.listen(PORT, () => {
  ...
  backfillRecordedInvoices()
    .then(() => backfillPackingListFiling())
    .then(() => db.saveToDisk())
